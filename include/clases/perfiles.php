...
class perfiles extends pagina{
	...
	private function getFormNPerfil(){
		...
		<div class="col-md-6 col-sm-12" id="formNPerfil" style="display:none">
            <div class="box-footer">
                ...
                <div class="box-body">
                    <form role="form">
                        <div class="form-group">
                            <label>Abreviacion (2 caracteres)</label>
                            <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Escribe la abreviación del perfil (2)" maxlength="2"/>
                        </div>
                        <div class="form-group">
                ...
                <button type="button" class="btn btn-primary" id="addPerfil">Guardar Nuevo Perfil</button>
	...
	private function getFormEPerfil(){
		...
		<div class="col-md-6 col-sm-12" id="formEPerfil" style="display:none">
            <div class="box-footer">
            	<div class="box-header">
                    <h3 class="box-title">Información Perfil</h3>
                </div>
                <div class="box-body">
                    <form role="form">
                        ...
                        <div class="form-group">
                            <label>Abreviacion (2 caracteres)</label>
                            <input type="text" class="form-control" id="nombrePerfil" name="nombrePerfil" maxlength="2"/>
                        </div>
                        <div class="form-group">
                            <label>Nombre Perfil</label>
                ...
                <button type="button" class="btn btn-primary" id="SendEditPerfil">Guardar Perfil</button>
                <button type="button" class="btn btn-primary hideForms">Cancelar</button>
            </div>
        </div>';
	...
	private function getSQL(){
		$sql = '
			SELECT * FROM in_niveles ';

			if($this->level != "1"){
				$sql .= ' WHERE status <> 9 AND id > 1';
			}
	...
	private function getBotones($tipo, $id){
		$dato = '
			<a href="#" class="editPerfil" id="'.$id.'"><i class="fa fa-edit"></i> Editar </a><br />
			<a href="#" class="elimPerfil" id="'.$id.'"><i class="fa fa-trash"></i> Eliminar </a><br />';

		if($tipo == 1){//Entra porque esta activo
			$dato .= '<a href="#" class="susPerfil" id="'.$id.'"><i class="fa fa-warning"></i> Suspender </a><br />';
		}
		elseif($tipo == 2){//Entra porque esta inactivo
			$dato .= '<a href="#" class="actPerfil" id="'.$id.'"><i class="fa fa-check"></i> Activar </a><br />';
		}
		elseif($tipo == 9 AND $this->level == "1"){
			$dato .= '<a href="#" class="actPerfil" id="'.$id.'"><i class="fa fa-check"></i> Re-Activar </a><br />';
		}
	...
	private function getTitulo(){
        ...
        <section class="content-header">
          <h1>
            Lista de Perfiles
            <small><button type="button" class="btn btn-primary" id="showPerfil">Nuevo Perfil</button></small>
          </h1>
          '.$this->getTimeLine().'
        </section>';
...
$pagina = new perfiles($section, $subsection);
